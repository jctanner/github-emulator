...
async def test_admin_runners_page_requires_admin_session(client):
    response = await client.get("/admin/runners", follow_redirects=False)
...
async def test_admin_runners_page_shows_scope_health_labels_and_current_job(
    ...
    response = await client.get("/admin/runners", cookies=admin_cookies())
    ...
    assert 'href="/admin/runners"' in response.text
