...
def _can_manage_repo(repo: Repository, current_user: Optional[User]) -> bool:
    """Return whether the current UI user can administer repository settings."""
    return bool(
        current_user
        and (current_user.site_admin or current_user.id == repo.owner_id)
    )
...
async def _repository_owner_choices(
    db: AsyncSession, current_user: User
) -> list[dict[str, str]]:
    """Return namespaces where the current user may create repositories."""
    ...
    return [
        {"login": current_user.login, "type": "User"},
        *(
            {"login": organization.login, "type": "Organization"}
            for organization in organizations
        ),
    ]


async def _resolve_repository_owner(
    db: AsyncSession,
    current_user: User,
    requested_owner: str,
) -> tuple[str, str]:
    """Validate an owner selection and return its namespace and type."""
    if requested_owner == current_user.login:
        return current_user.login, "User"
    result = await db.execute(
        select(Organization)
        .join(OrgMembership, OrgMembership.org_id == Organization.id)
        .where(
            Organization.login == requested_owner,
            OrgMembership.user_id == current_user.id,
            OrgMembership.state == "active",
        )
    )
    ...
    if organization is None:
        raise ValueError(
            f"You cannot create repositories under '{requested_owner}'."
        )
...
async def new_repo_page(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        ...
        context=_ctx(
            ...
            owner_choices=await _repository_owner_choices(db, current_user),
            selected_owner=current_user.login,
            error=None,
        ),
    )
...
async def new_repo_submit(
    ...
    try:
        namespace_login, owner_type = await _resolve_repository_owner(
            db,
            current_user,
            owner.strip() or current_user.login,
        )
        ...
        return templates.TemplateResponse(
            ...
            context=_ctx(
                ...
                current_user=current_user,
                owner_choices=await _repository_owner_choices(db, current_user),
                selected_owner=owner.strip() or current_user.login,
                error=str(exc),
            ),
        )
...
# Feature routers are imported after shared helpers are defined to keep this
# module a backward-compatible facade for existing helper imports.
